...
| `shared/review_fanout.py` | Per-file × dimension review fanout for `REVIEW:` tasks — complexity gating (trivial/moderate/complex) for dimension selection, **LOC × dimension-reasoning-weight per-agent tiering** (`tier_for(loc=...)`: <230→low, 230–600→medium, >600 reasoning-heavy→high, security+risk→high), **explicit `[dims=...]` intent** (`_requested_dimensions`, runs only named dims with drop-protection + security-on-risk), scaled `synthesis_tier`, `build_review_subtasks()` |
