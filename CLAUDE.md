...
| `shared/review_fanout.py` | Per-file × dimension review fanout for `REVIEW:` tasks — complexity gating (trivial/moderate/complex) for dimension selection, **LOC + structural-density per-agent tiering** (`tier_for(loc=, density_score=, bias=)`: <230→low, 230–600→medium, >600 reasoning-heavy→high, dense mid-size reasoning-heavy→high, flat large→medium, security+risk→high), **explicit `[dims=...]` intent** (`_requested_dimensions`, runs only named dims with drop-protection + security-on-risk), scaled `synthesis_tier`, `profile_key_for()` (transferable `ext\|loc_bucket\|density_bucket` learning key), `build_review_subtasks(tier_bias=)` |
| `shared/review_learning.py` | Profile-keyed review-tier learning loop (cold/warm only, no LLM, no spawn-path cost). Review agents are read-only so the signal is FINDINGS not rework: `record_review_tier_outcome()` EMA-updates `review_tier_bias` (escalate when a cheap tier kept high-severity findings, de-escalate when a high tier idled); `load_review_tier_bias()` returns a clamped `{(profile_key, dim): ±1}` map read once at plan-build. Empty/no-data → pure heuristic (fresh-repo safe). Gated by `review_learning_enabled`. Captured in `host_learning` finalize, read in `heuristic_plan` |
...
- **Learning is approval-gated**: pattern tracking in `shared/agents.py` flows through draft readiness and the approval queue before local activation or cross-CLI registration. Do not bypass the approval step. On approval, `approval_queue_approve` branches on draft `kind`: agent drafts best-effort export to `.claude/skills/<slug>/` via `export_agent_skill`; `kind="workflow"` drafts skip the agent save path entirely and export to `.claude/workflows/<slug>.js` via `export_workflow(tune=True)`. Both exports are non-blocking — a failure never undoes the approval. Learned agents are auto-assigned to decompose-path subtasks via `AgentRegistry` (instantiated in `mcp_server` `_ensure_init`, passed to `Planner`); matching is string-similarity only (no LLM).
...
- Host-native heuristic planning (`shared/heuristic_plan.py`) fans out one agent per file for webapp/fullstack intent; tasks starting with `REVIEW:` route to `shared/review_fanout.py` for per-file × dimension review DAG (optional `[dims=...]` token names the dimensions; it is stripped before file extraction so it is never read as a path). Per-agent tier is chosen from raw LOC × dimension reasoning-weight, so a review mixes low/medium/high agents rather than all-medium. For `REVIEW:` runs the host-native `execute_swarm` response is trimmed to a compact spawn manifest (`host_spawn_waves` + `plan_summary`, no duplicate `plan`/`workflow_script`) so first-spawn stays fast; full plan fidelity is still recorded in the run receipt (`inspect_run_receipt`). Mid-run `expand_host_plan` adds agents for discovered files. Learning ingest merges handoff snapshots in `shared/host_learning.py`
